...
import os
import pandas as pd
from PyQt6.QtCore import (Qt, QDir, QItemSelectionModel, QAbstractTableModel, 
                          QModelIndex, QVariant, QSize, QSettings)
from PyQt6.QtWidgets import (QMainWindow, QTableView, QApplication, QToolBar, QLineEdit, QComboBox, QDialog, 
                                                            QMenu, QFileDialog, QAbstractItemView, QMessageBox, QWidget)
from PyQt6.QtGui import (QStandardItemModel, QStandardItem, QCursor, QIcon, QAction, 
                         QKeySequence, QTextDocument, QTextCursor, QTextTableFormat)
...
class Viewer(QMainWindow):
    def __init__(self, parent=None):
      ...
      self.lb.setGridStyle(Qt.PenStyle.SolidLine)
      self.model =  PandasModel()
      self.lb.setModel(self.model)
      self.lb.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked)
      self.lb.setSelectionBehavior(self.lb.SelectionBehavior.SelectRows)
      self.lb.setSelectionMode(self.lb.SelectionMode.SingleSelection)
    ...
    def createToolBar(self):
        openAction = QAction(QIcon.fromTheme("document-open"), "Open",  self, triggered=self.loadCSV, shortcut = QKeySequence.StandardKey.Open)
        saveAction = QAction(QIcon.fromTheme("document-save"), "Save", self, triggered= self.writeCSV_update, shortcut = QKeySequence.StandardKey.Save) 
        saveAsAction = QAction(QIcon.fromTheme("document-save-as"), "Save as ...", self,  triggered=self.writeCSV, shortcut = QKeySequence.StandardKey.SaveAs) 
        ...
        self.lineFind.addAction(findbyText, QLineEdit.ActionPosition.LeadingPosition)
